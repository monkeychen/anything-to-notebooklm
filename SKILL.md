...
| 输入特征 | 识别为 | 处理方式 |
|---------|-------|---------|
| `https://mp.weixin.qq.com/s/` 或 `https://mp.weixin.qq.com/s?` | 微信公众号 | MCP 工具抓取 |
...
**微信公众号**：
- 默认情况：使用 MCP 工具 `read_weixin_article` 获取文本，保存为 TXT：`/tmp/weixin_{title}_{timestamp}.txt`
- 如果用户要求转为PDF：使用 MCP 工具 `save_weixin_article_to_pdf` 生成包含图片的完整PDF，保存为：`/tmp/weixin_{title}_{timestamp}.pdf`
...
### Step 4: 清除临时文件
当所有内容上传到 NotebookLM 后，建议清除临时文件以释放空间。
```bash
rm /tmp/*.txt # 删除所有生成临时的 TXT 文件
rm /tmp/*.pdf # 删除所有生成的临时的 PDF 文件
rm /tmp/*.json # 删除所有生成的临时的 JSON 文件（如思维导图）
```
